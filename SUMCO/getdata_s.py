...
for x in range(len(l_list_s['KEY2'])):
    ...
    if df0['DB接続名'].values =='ROWDB_R3_240':
        ...
        df2.to_csv('SUMCO/'+str(l_list_s['KEY2'][x]+'/'+l_list_s['KEY2'][x])+'_個数.csv',encoding='SHIFT-JIS',index=False,header=True)
    #239とつなぐ場合
        ...
        df2 = df2.loc[:,['ﾒｯｷ後カメラ1ダイヤ個数','ﾒｯｷ後カメラ2ダイヤ個数','ﾒｯｷ後カメラ3ダイヤ個数','ﾒｯｷ後カメラ1_3平均個数']]
        df2.to_csv(str(l_list_s['KEY2'][x]+'/'+l_list_s['KEY2'][x])+'_個数.csv',encoding='SHIFT-JIS',index=False,header=True)
    else :
        print('no_data')
    #＿＿＿線径データ______
    #線径履歴から開始日時と終了日時を取得
    ...
    needs.to_csv('SUMCO/'+l_list_s['KEY2'][x]+'/'+str(l_list_s['KEY2'][x])+'_線径.csv', index=False, encoding = "SHIFT-JIS")
