...
#ファイルの存在確認-SUMCO-
if os.path.exists('SUMCO/sumco.csv'):
...
for x in range(len(l_list_s['KEY2'])):
    try:
        os.mkdir('SUMCO/'+l_list_s['KEY2'][x])
    except Exception:
        pass
    #LotNoでループ
    query0 = 'declare @製造番号 nvarchar(50) =' + "'" + str(l_list_s['KEY2'][x]) + "';"+ '''
    ...
    if df0['DB接続名'].values =='ROWDB_R3_240':
        ...
        df2.to_csv('SUMCO/'+str(l_list_s['KEY2'][x])+'/'+'dia_count.csv',encoding='SHIFT-JIS',index=False,header=True)
    #239とつなぐ場合
    ...
    #＿＿＿線径データ______
    #線径履歴から開始日時と終了日時を取得
    query2 = " SELECT * FROM ROWDB.dbo.巻替え_線径履歴 WHERE 製造番号 ="  + "'" +str(l_list_s['KEY2'][x]) + "'"
    df2 = pd.read_sql(
                    query2
                    ,conn2
                    )
    ...
    data.to_csv('SUMCO/'+l_list_s['KEY2'][x]+'/'+str(l_list_s['KEY2'][x])+ '.csv',encoding='SHIFT-JIS')
    #必要な列数だけにする
    needs = pd.read_csv('SUMCO/'+l_list_s['KEY2'][x]+'/'+str(l_list_s['KEY2'][x])+ '.csv', encoding = "SHIFT-JIS")
    needs.drop(columns = ['Unnamed: 0'], inplace=True)
    needs.columns =['ID','履歴ID','日時','線径μm']
    needs.to_csv('SUMCO/'+l_list_s['KEY2'][x]+'/'+'diameter.csv', index=False, encoding = "SHIFT-JIS")
    os.remove('SUMCO/'+l_list_s['KEY2'][x]+'/'+str(l_list_s['KEY2'][x]) + '.csv')
    #zip圧縮
    shutil.make_archive('SUMCO/'+l_list_s['KEY2'][x]+'/'+str(l_list_s['KEY2'][x]), 'zip', root_dir='SUMCO/'+str(l_list_s['KEY2'][x]))
    ...
    # アップロードするファイルパス
    upload_src_path = '三益/'+l_list_s['KEY2'][x]+'/'+str(l_list_s['KEY2'][x])+'.zip'
    # アップロード先のファイルパス（STORはファイルをアップロードするためのFTPコマンドなので必要です。）
    upload_dst_path = "STOR "+ str(l_list_s['KEY2'][x]) + '.zip'
    # サーバーのユーザー名
    username = "test01" 
    # サーバーのログインパスワード
    password = "test01" 
